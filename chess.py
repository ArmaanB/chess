...
def render(screen,board,selected_tile):
    ...
    for row in range(8):
        for col in range(8):
            ...
            if col % 2 != row % 2:
                color = DARK_BROWN
            if selected_tile and selected_tile[0] == col and selected_tile[1] == row:
                color = YELLOW
            pygame.draw.rect(screen,color,(60 + col * 60,60 + row * 60,60,60))

            if board[row][col] != 0:
                screen.blit(PIECE_IMAGES[board[row][col]],(60 + col * 60,60 + row * 60))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if board[index[1]][index[0]] != 0 and board[index[1]][index[0]] <= 6:
                    selected_tile = index
                else:
                    selected_tile = None
